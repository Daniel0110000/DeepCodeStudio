...
class ParameterizedCompletionContext {
	 ...
	 */
	private HighlightPainter p;

	private HighlightPainter endingP;

	private HighlightPainter paramCopyP;
	...
	private void possiblyUpdateParamCopies(Document doc) {
		...
		if (index>-1 && index<pc.getParamCount()) {
			...
			ParameterizedCompletion.Parameter param = pc.getParam(index);
			if (param.isEndParam()) {
				deactivate();
				return;
			}
